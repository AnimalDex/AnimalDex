...
def load_model(): 
  model = tf.keras.models.load_model('backend/model_ad.h5')
  model.summary()
  return model
...
def show_three_best(score):
  ...
  write_output2(resultado_nombre, resultado_porcentaje)


def write_output(resultado_nombre, resultado_porcentaje):
  with open('backend/output.csv', 'w', newline='') as csvfile:
...
def write_output2(resultado_nombre, resultado_porcentaje):
  with open('backend/output.csv', 'w') as f:
    f.write(resultado_nombre[0])
...
def main():
  print(get_all_images('backend/images'))
  images = get_all_images('backend/images')
  model = load_model()
